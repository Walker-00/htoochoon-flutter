          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: const Color(0xFF1c2e4a).withValues(alpha: 0.2),
                    blurRadius: 60,
                    spreadRadius: 10,
...
class _AuthFormSectionState extends State<_AuthFormSection> {
  ...
  void _handleSubmit(AuthProvider provider, BuildContext context) async {
    ...
    if (_isSignUp) {
      ...
      final result = await provider.register(registerRequest, context);
      ...
      if (result == RegisterResult.success) {
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (context) =>
                OtpScreen(email: registerRequest.email, isRegister: true),
          ),
        );
      } else {
      ...
      if (result == LoginResult.success) {
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (context) =>
                OtpScreen(email: loginRequest.email, isRegister: false),
          ),
        );
      } else {
  ...
  //
  //         //
  //         // logD("STATUS: ${result.statusCode}");
  //         // logD("DATA: ${result.data}");
  //         if (!context.mounted) return;
  //
  //         if (result == LoginResult.success) {
  //           Navigator.pushAndRemoveUntil(
...
class _LogoBranding extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Align(
      ...
      child: PeacockLogoLockup(markSize: 88, title: 'Htoo Choon'),
    );
  }
}
...
/// Terms + Privacy notice shown under the sign-in / sign-up button. No checkbox:
/// continuing (signing in or up) is treated as acceptance, per product spec.
