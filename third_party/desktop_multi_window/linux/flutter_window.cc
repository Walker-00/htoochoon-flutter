...
gboolean on_close_clicked(GtkWidget *widget, GdkEvent *event, gpointer user_data) {
    // LOCAL PATCH: hide instead of destroy.
    // Destroying a sub-window's Flutter engine on Linux tears down the whole
    // process — closing the whiteboard (titlebar X, or a WM "close active"
    // bind like Hyprland Super+Shift+Q) would kill the main app + live meeting.
    // Hiding keeps the engine alive; the OS process exit reclaims it. Closing
    // the whiteboard must never close the main app.
